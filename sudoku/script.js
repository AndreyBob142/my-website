...
function initNumbers() {
    ...
    numbers.forEach(number => {
        number.addEventListener('click', () => omNamberClick(parseInt(number.innerHTML)))
    });
}

function omNamberClick(number) {
...
function setValueInSelectedCell(value) {
    ...
    const duplicatesPositions = sudoku.getDuplicatePosition(row, column, value);
...
function initKeyEvent() {
    document.addEventListener('keydown', event => {
        if (event.key === 'Backspace') {
            ...
            omNamberClick(parseInt(event.key));
        }
    });
}
